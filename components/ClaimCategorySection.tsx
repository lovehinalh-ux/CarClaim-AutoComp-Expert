    ...
    <section className="bg-white rounded-3xl shadow-sm border border-stone-100 overflow-hidden mb-8">
      ...
      <div className="divide-y divide-stone-100">
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={`p-6 md:p-8 transition-colors hover:bg-stone-50/80 ${hasError ? 'bg-amber-50/50' : ''}`}>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`text-lg font-bold ${hasError ? 'text-amber-800' : 'text-stone-900'}`}>{item.name}</span>
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-3 w-full md:w-auto bg-stone-50 rounded-xl p-1 border border-stone-200 focus-within:border-stone-900 focus-within:ring-1 focus-within:ring-stone-900 transition-all">
                    <span className={`pl-4 font-black text-lg ${hasError ? 'text-amber-600' : 'text-stone-400'}`}>NT$</span>
                    <input
                      type="number"
                      min="0"
